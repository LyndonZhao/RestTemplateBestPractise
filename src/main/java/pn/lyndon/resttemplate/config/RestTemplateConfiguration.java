package pn.lyndon.resttemplate.config;
...
public class RestTemplateConfiguration {
    ...
    // 读取超时默认3s
    @Value("${http.config.readTimeout:30000}")
    private int readTimeout;
    // 读取超时默认3s
    @Value("${http.config.connectionRequestTimeout:500}")
    ...
    private HttpRequestRetryHandler getHttpRequestRetryHandler() {
        return (IOException exception, int executionCount, HttpContext context) -> {
            ...
            return context.getAttribute(HTTP_REQUEST) instanceof HttpEntityEnclosingRequest;
        };
    }
